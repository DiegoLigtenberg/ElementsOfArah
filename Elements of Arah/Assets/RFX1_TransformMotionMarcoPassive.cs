...
public class RFX1_TransformMotionMarcoPassive : MonoBehaviour
{
    ...
    public AE_PhysicsMotion ae;
    void Update()
    {
        Debug.Log(ae.IMPACT);
        if (ae.IMPACT != null)
        {
            Target = ae.IMPACT.gameObject;
            targetT = ae.IMPACT.transform;
        }
        ...
        if (timer <= 0)
        {
            ...
            if (velocity < 1)
            {
                ...
                if (counter >=3)
                {
                    ...
                    {
                        var instance = Instantiate(effect, transform.position * CollisionOffset, new Quaternion()) as GameObject;
                        ...
                        instance.transform.LookAt(transform.position * CollisionOffset);
    ...
    void UpdateWorldPosition()
    {
        ...
        RaycastHit hit;
        if (!isCollided && Physics.SphereCast(t.position,0.1f, frameMoveOffsetWorld.normalized, out hit, Distance, CollidesWith))
        {
