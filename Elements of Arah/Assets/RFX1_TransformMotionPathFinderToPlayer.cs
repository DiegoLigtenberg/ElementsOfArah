...
public class RFX1_TransformMotionPathFinderToPlayer : MonoBehaviour
{
            ...
            else
            {
                ...
                if (Time.time - lastStep > timeBetweenSteps)
                {
                  ...
                  //  if (i <3)
                    ...
                    if (i >=3)
                    {
                        timeBetweenSteps = timeBetweenSteps + 5.41f; //;  + 5.41f;
                        lastStep = Time.time;
                        forwardVec = (new Vector3(targetT.position.x, 64.7f, targetT.position.z) - t.position).normalized;
                    }
                    */
        ...
        CollidedInstances.Clear();
            foreach (var effect in EffectsOnCollision)
            {
                ...
                if (GameObject.Find("heraklios_a_dizon@Jumping (2)").GetComponent<Animator>().GetBool("isGrounded")) {
                    ...
                    // DIT STOND EERST AAN MISSCHIEN TERUG PROBEREN ALS T FAALT
